...
describe("createPageWithBlocks", () => {
  ...
  const pageId = await createPageWithBlocks(mockClient, "parent-id", "Test Page", blocks);
...
describe("replacePageBlocks", () => {
  ...
  expect(mockClient.blocks.delete).toHaveBeenCalledWith({ block_id: "block-2" });
  expect(mockClient.blocks.delete).toHaveBeenCalledWith({ block_id: "block-3" });
  ...
  const existingBlockIds = Array(250).fill(null).map((_, i) => `block-${i}`);
  const mockClient = {
    blocks: {
      children: {
        list: mock(() =>
