...
describe('parseInline', () => {
  ...
  test('plain text', () => {
    ...
    expect(result).toEqual([
      { type: 'text', text: 'Plain text with no formatting.' },
    ]);
  });
  ...
  test('multiple links', () => {
    ...
    const result = parseInline(
      'See [link1](https://example1.com) and [link2](https://example2.com).'
    );
    expect(result).toEqual([
      { type: 'text', text: 'See ' },
...
describe('parseInline', () => {
  test('link with special characters', () => {
    const result = parseInline(
      'Visit [Example](https://example.com/page?q=test&id=123).'
    );
    expect(result).toEqual([
      { type: 'text', text: 'Visit ' },
      {
        type: 'text',
        text: 'Example',
        link: 'https://example.com/page?q=test&id=123',
      },
      { type: 'text', text: '.' },
    ]);
  });
