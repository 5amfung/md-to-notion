...
describe("importMarkdown", () => {
  test("dry run mode creates no files", async () => {
    ...
    await importMarkdown(scan, "parent-page-id", { dryRun: true, verbose: true });
  ...
  test("skips unchanged files based on hash", async () => {
    ...
    const fileHash = createHash("sha256").update(Buffer.from(fileContent)).digest("hex");
  ...
  test("force updates even when hash matches", async () => {
    ...
    await importMarkdown(scan, "parent-page-id", { force: true, dryRun: true, verbose: true });
  ...
  test("detects content changes via hash", async () => {
    ...
    const updatedState = await loadSyncState("parent-page-id");
    expect(updatedState.files["test.md"]?.notionPageId).toBe("existing-page-id");
  });
  ...
  test("createRootPage false creates file under destination", async () => {
    ...
    await importMarkdown(scan, "dest-page-id", { notionClient: notion, verbose: true });
  ...
  test("update existing page (replacePageBlocks)", async () => {
    ...
    const notion = createMockNotionClient();
    (notion.blocks.children.list as ReturnType<typeof mock>)?.mockResolvedValueOnce({
      results: [{ id: "block-1" }, { id: "block-2" }],
      has_more: false,
    });

    await importMarkdown(scan, "dest-page-id", { notionClient: notion, force: true, verbose: true });
  ...
  test("skip logs when verbose", async () => {
    ...
    const { createHash } = await import("crypto");
    const fileHash = createHash("sha256").update(Buffer.from("# Content")).digest("hex");
    const state = await loadSyncState("dest-page-id");
    state.files["test.md"] = { notionPageId: "id", contentHash: fileHash, lastSynced: new Date().toISOString() };
    await saveSyncState(state);
    ...
    try {
      await importMarkdown(scan, "dest-page-id", { verbose: true, notionClient: createMockNotionClient() });
