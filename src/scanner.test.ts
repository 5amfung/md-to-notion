...
describe("scanInput", () => {
  ...
  test("directory with nested markdown files", async () => {
    ...
    expect(result.mdFiles).toContain(join(testDir, "subdir", "nested", "file3.md"));
  ...
  test("throws when path is neither file nor directory", async () => {
    ...
    await expect(scanInput(fifoPath)).rejects.toThrow("not a file or directory");
  });
...
describe("scanMultipleInputs", () => {
  ...
  test("empty input paths error", async () => {
    await expect(scanMultipleInputs([])).rejects.toThrow("No input paths provided");
  });
  ...
  test("no valid paths when all are .notion-sync.json", async () => {
    ...
    await expect(
      scanMultipleInputs([join(dir1, ".notion-sync.json"), join(dir2, ".notion-sync.json")])
  ...
  test("walks up for common parent when paths are siblings", async () => {
    ...
    const result = await scanMultipleInputs([join(aa, "a.md"), join(bb, "b.md")]);
